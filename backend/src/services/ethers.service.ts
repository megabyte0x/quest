...
import { CONTRACT_ADDRESSES, CONTRACT_ADDRESSES_TXNX, TOKEN_IDS } from "../config/contract";
import { ACCOUNT_FUNCTION_SIGNATURE, BALANCE_OF_FUNCTION_SIGNATURE } from "../config/signature";
...
export const getAccTransactionData = async (tokenId: string) => {
    try {
        ...
        const params = [
            CONTRACT_ADDRESSES_TXNX[3],
            chainId,
            CONTRACT_ADDRESSES_TXNX[2],
            tokenIdAsBigInt,
            salt,
            initData,
        ];
        ...
        const transactionGasPrice = await provider.estimateGas({
            to: CONTRACT_ADDRESSES_TXNX[0],
            data: data,
        });

        const transactionData = {
            to: CONTRACT_ADDRESSES_TXNX[2],
            data: data,
...
export const getAssetTransactionData = async (address: string, tkbAddress: string) => {
